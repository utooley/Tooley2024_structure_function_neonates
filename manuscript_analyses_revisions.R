...
# Brainsmash: Does SC-FC variation correspond to the S-A axis----------------------------------
#Tests from https://brainsmash.readthedocs.io
#need a brain map and a distance matrix, the example distance matrix is that they use is geodesic distance

#gordon_geodesic_dist_largexhem_Conte69_wb.mat is the geodesic distances between parcels in the Gordon parcellation 
# Ashley N. made this, can use it

library(R.matlab)
distances_folder= "~/Box/projects/in_progress/struct_funct_neonates/data/clean_data_july_2023/brainsmash_spin_tests/"
geodesic <- readMat(paste0(distances_folder, "gordon_geodesic_dist_largexhem_Conte69_wb.mat"))

#save these dist mats both out as lh and rh delimited txt files to use in brainsmash
write.table(geodesic$distmat[1:161,1:161], file = paste0(distances_folder,"gordon_geodesic_distMat_lh.txt"),
            col.names = F, row.names = F)
write.table(geodesic$distmat[162:333,162:333], file = paste0(distances_folder,"gordon_geodesic_distMat_rh.txt"),
            col.names = F, row.names = F)

#write out scfc files to use in brainsmash
write.table(scfc.norm.roi.thresh75$mean.regional.sc.fc, file = paste0(distances_folder,"scfc_norm_roi_thresh75_mean_scfc.txt"),
            col.names = F, row.names = F)

#write out 324 versions
geodesic.324 <- geodesic$distmat[-indices_to_remove, -indices_to_remove]
Euclidean.324 <- Euclidean[-indices_to_remove, -indices_to_remove]
#save these dist mats both out as lh and rh delimited txt files to use in brainsmash
write.table(geodesic.324[1:157,1:157], file = paste0(distances_folder,"gordon_geodesic_distMat_324_lh.txt"),
            col.names = F, row.names = F)
write.table(geodesic.324[158:324,158:324], file = paste0(distances_folder,"gordon_geodesic_distMat_324_rh.txt"),
            col.names = F, row.names = F)
write.table(scfc.norm.roi.thresh75.324$mean.regional.sc.fc, file = paste0(distances_folder,"scfc_norm_roi_thresh75_mean_scfc_324.txt"),
            col.names = F, row.names = F)
...
#write it out to look at with brainsmash spin tests
write.table(data_scfc$parcel_age_betas, file = paste0(distances_folder,"scfc_norm_roi_thresh75_age_effects.txt"),
            col.names = F, row.names = F)
...
#delta r-squared
nullmodel <- lm(part_coef_avg~child_sex+Relative_Motion+intracranial_volume, data=sc_network_metrics_matlab) #total_edges, intracranial volume
summary(sc_part_coef_model)$r.squared - summary(nullmodel)$r.squared
...
#delta r-squared
nullmodel <- lm(modul~child_sex+Relative_Motion+intracranial_volume, data=sc_network_metrics_matlab) #total_edges, intracranial volume
summary(sc_modul_model)$r.squared - summary(nullmodel)$r.squared

#correlation between modul and part coef
cor.test(sc_network_metrics_matlab$modul, sc_network_metrics_matlab$part_coef_avg)
library(ppcor)
pcor.test(sc_network_metrics_matlab$modul, sc_network_metrics_matlab$part_coef_avg, sc_network_metrics_matlab$avgweight)
...
#participation coefficient with avg weight
...
#delta r-squared
nullmodel <- lm(avg_part_coef~child_sex+avg_FD_of_retained_frames+retained_frames+avgweight, data=y0_network_metrics_matlab) #total_edges, intracranial volume
summary(fc_part_coef_model)$r.squared - summary(nullmodel)$r.squared

#modularity with avg weight
...
#delta r-squared
nullmodel <- lm(modul~child_sex+avg_FD_of_retained_frames+retained_frames+avgweight, data=y0_network_metrics_matlab) #total_edges, intracranial volume
summary(fc_modul_model)$r.squared - summary(nullmodel)$r.squared

#correlation between modul and part coef
cor.test(y0_network_metrics_matlab$modul, y0_network_metrics_matlab$avg_part_coef)
pcor.test(y0_network_metrics_matlab$modul, y0_network_metrics_matlab$avg_part_coef, y0_network_metrics_matlab$avgweight)
...
sc_fc_eyetrack_y1 %>% filter(!is.na(Avg.Center.Peripheral.TTFF) & !is.na(avg_fpn_scfc)) %>% dplyr::select(modid) %>% dim()
...
#Delta adjusted r-squared as a measure of variance explained for reviewer
model <- TTFF.avg.model; nullmodel <- lm(Avg.Center.Peripheral.TTFF~ child_age_y1_assessment+percent_valid, data=sc_fc_eyetrack_y1)
summary(model)$r.squared - summary(nullmodel)$r.squared
...
for (network in networks){
  ...
  model <- l; nullmodel <- lm(Avg.Center.Peripheral.TTFF~ child_age_y1_assessment+percent_valid, data=sc_fc_eyetrack_y1)
  delta.r.squared[network] <-   summary(model)$r.squared - summary(nullmodel)$r.squared
  visreg(l, network ,ylab= "Average TTFF", xlab = network, main = paste(network, "p =", round(summary(l)$coefficients[4,4],2)))
}
unlist(pvals)
pvals <- data.frame(cbind(networks,unlist(pvals),p.adjust(unlist(pvals),method = "fdr"), unlist(delta.r.squared)))
pvals
